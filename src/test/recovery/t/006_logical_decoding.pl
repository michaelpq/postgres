...
# Move the OID counter past 2^32, so that the oid8 TOAST values decoded at
# the end of this test have IDs that do not fit in 4 bytes.
...
# Decode an oid8 TOAST value whose ID is past 2^32.  The reorder buffer
# collects the chunks by chunk_id, and has to read that column with the
# TOAST table's type, or the value is reported as unchanged.
