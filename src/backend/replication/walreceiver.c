...
WalReceiverMain(const void *startup_data, size_t startup_data_len)
{
	...
	walrcv->ready_to_display = false; //here
	 ...
	 * a attempting to contact the primary server.  Note that this resets
	 * the original conninfo and sender_host, for security.  These fields
	 * are filled once the connection is fully established.
	 */
	SpinLockAcquire(&walrcv->mutex);
	...
	/*
	 * Save user-visible connection string, now that the connection has
	 * been achieved.
	 */
