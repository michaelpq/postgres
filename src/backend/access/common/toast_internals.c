					   ...
					   varlena *oldexternal, int32 *data_todo)
{
	 ...
	 * table.  Since that is the very same relation, its chunk_id type
	 * necessarily matches, so no separate vartag check is needed.
	 */
				 ...
				 varlena *oldexternal, uint32 options)
{
	 ...
	 * toast_preserve_valueid); otherwise we pick a fresh one. For Oid value
	 * IDs the fresh one must not conflict with old or new toast values; for
	 * Oid8 the ID space is large enough that conflicts are not a concern.
	 */
 ...
 */
void
toast_valueid_scankey_init(ScanKey entry, AttrNumber attno,
						   Oid toast_typid, Oid8 valueid)
{
	Assert(toast_typid == OIDOID || toast_typid == OID8OID);
	if (toast_typid == OID8OID)
		ScanKeyInit(entry, attno,
...
toast_delete_datum(Relation rel, Datum value, bool is_speculative)
{
	...
	/* Set up a scan key to find chunks with matching value ID */
	toast_valueid_scankey_init(&toastkey, (AttrNumber) 1,
...
toastrel_valueid_exists(Relation toastrel, Oid8 valueid)
{
	 ...
	 */
	toast_valueid_scankey_init(&toastkey, (AttrNumber) 1, toast_typid,
							   valueid);
