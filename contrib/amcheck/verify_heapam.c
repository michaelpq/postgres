				  ...
				  uint32 extsize)
{
	...
	if (toast_typid == OIDOID)
		toast_valueid = ta->toast_pointer.value_oid;
	else if (toast_typid == INT8OID)
		toast_valueid = ta->toast_pointer.value_uint64;
...
check_tuple_attribute(HeapCheckContext *ctx)
{
	...
	if (VARATT_IS_EXTERNAL_ONDISK_OID(attr))
		toast_pointer_valueid = toast_pointer.va_valueid_oid;
	else if (VARATT_IS_EXTERNAL_ONDISK_INT8(attr))
		toast_pointer_valueid = toast_pointer.va_valueid_uint64;
	...
	if (!ctx->tuple_could_be_pruned)
	{
		...
		varatt_external_get(attr, ta->toast_pointer);
		ta->blkno = ctx->blkno;
...
check_toasted_attribute(HeapCheckContext *ctx, ToastedAttribute *ta)
{
	...
	if (toast_typid == OIDOID)
		ScanKeyInit(&toastkey,
					...
					ObjectIdGetDatum(ta->toast_pointer.value_oid));
	else (toast_typid == INT8OID)
		ScanKeyInit(&toastkey,
					(AttrNumber) 1,
					BTEqualStrategyNumber, F_INT8EQ,
					Int64GetDatum(ta->toast_pointer.value_uint64));
	else
		Assert(false);
