...
-- Check TOAST relations of both chunk_id types.  The pointer decoding, the
-- maximum chunk size and the TOAST index lookup all depend on it.
...
-- Uncompressed out-of-line values, then compressed ones.
...
INSERT INTO test_toast_oid8 (a, b)
	(SELECT gs, repeat('xyzzy', 20000) FROM generate_series(6,10) gs);
-- Each value spans many chunks, so chunk_seq gets exercised as well.
...
-- No corruption expected in either case.
